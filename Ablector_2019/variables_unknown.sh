...
BENCHMARK_LIST="unknown_mul_xdiv_xrem.txt"
# Additional arguments for the binary
ARGS="--ufReuse=1 --unsigned"
...
if [ `hostname` = "hal9000" ]; then
    ...
    LOG_DIR_BASE=`pwd`"/logs/Ablector_2019/unknown/"
    ...
    EXPERIMENT_DIR=`pwd`
    PAR_NUM=2
    SOURCE_SCRIPT=`pwd`"/Ablector_2019/source.sh"
    ...
    # Directory for log writing
    LOG_DIR_BASE="/raid/steuber/Logs/Ablector_2019/unknown/"
    # Binary which should be executed
